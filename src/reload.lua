...
function generateForcedEncounterData(currentRun, room, args)
	...
	elseif room.IsEncounterAutoGenerated then
...
function MyHandleEnemySpawns(encounter)
	...
	if encounter.CanEncounterSkip then
		...
		if roomParameters and roomParameters.Encounters and roomParameters.Encounters[GlobalRoomEncounterDepth] then
			if roomParameters.Encounters[GlobalRoomEncounterDepth].ForceFigSkipEncounterNumber then
				encounter.SpawnsSkipped = true
			end
		end
	end

	-- Increment the current encounter depth of the room, but not for Fields rooms (only for "cage" real Encounters)
	if not string.find(encounter.Name, 'GeneratedH_Passive') then
...
function _populateRoomData(biomeName, roomDepth, roomNumber)
	...
	elseif string.find(roomParameters.Name, '_Combat') and biomeName ~= 'H' then
		-- Auto generated encounter (GeneratedX) except for Fields(H) Biome
		createdRoomData.IsEncounterAutoGenerated = true
	...
	createdRoomData.SpawnWaves = roomParameters.SpawnWaves --@todo remove since it's not stored in the root of parameters anymore
